...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %% Feed Foward
  hid_input = model.input_to_hid * data.inputs; % size: <number of hidden units> by <number of data cases>
  hid_output = logistic(hid_input); % size: <number of hidden units> by <number of data cases>
  class_input = model.hid_to_class * hid_output; % size: <number of classes, i.e. 10> by <number of data cases>
  
  class_normalizer = log_sum_exp_over_rows(class_input); % size: <1> by <number of data cases>
  log_class_prob = class_input - repmat(class_normalizer, [size(class_input, 1), 1]); % size: <number of classes, i.e. 10> by <number of data cases>
  class_prob = exp(log_class_prob); % probability of each class. Each column (i.e. each case) sums to 1. size: <number of classes, i.e. 10> by <number of data cases>
  
  data_size = size(data.inputs, 2);
  
  %% Back Propagation
  % First, Compute deltas for each layer
  delta_2 = class_prob - data.targets; % size: <number of classes, i.e. 10> by <number of data cases>
  delta_1 = (model.hid_to_class' * delta_2) .* (logistic(hid_input) .* (1 - logistic(hid_input))); % size: <number of hidden units> by <number of data cases>
  
  % Compute gradient
  ret.hid_to_class = (1 / data_size) .* (delta_2 * hid_output') + (wd_coefficient .* model.hid_to_class); % size: <number of classes i.e. 10> by <number of hidden units>
  ret.input_to_hid = (1 / data_size) .* (delta_1 * data.inputs') + (wd_coefficient .* model.input_to_hid); % size: <number of hidden units> by <number of inputs i.e. 256>
end
